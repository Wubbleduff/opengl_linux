...
void init_graphics()
{
    ...
    graphics->window_width  = 400;
    graphics->window_height = 300;
    ...
    // Shaders
    {
        const char *vertex_shader_source =
            ...
            "void main()\n"
            "{\n"
            "    gl_Position = vec4(aPos.x, aPos.y, aPos.z, 1.0) + vec4(pos, 0.0);\n"
            "}\0";
...
void render()
{
    ...
    if(graphics->keys_down[25]) graphics->y += 0.01f;
    if(graphics->keys_down[38]) graphics->x -= 0.01f;
    if(graphics->keys_down[39]) graphics->y -= 0.01f;
    if(graphics->keys_down[40]) graphics->x += 0.01f;

    gl_fn->glUseProgram(graphics->shader_program);
    check_gl_errors("use shader program");

    GLint loc = gl_fn->glGetUniformLocation(graphics->shader_program, "pos");
    gl_fn->glUniform3f(loc, graphics->x, graphics->y, graphics->z);
